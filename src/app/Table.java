...
public class Table {
    ...
    private static DefaultTableModel getTable_DEF(String text) {
        ...
        Object[][] gridData = {
            {"Length", Parse.getSize(text)},
            {"Word Total", Parse.getWords(text)},
            {"Word Unique", Parse.getWordsUnique(text)},
            {"Characters", Parse.getCharacters(text)},
            {"Letters", Parse.getLetters(text)},
            {"UpperCase", Parse.getLettersUpper(text)},
            {"LowerCase", Parse.getLettersLower(text)},
            {"Numbers", Parse.getDigits(text)},
            {"Symbols", Parse.getSymbols(text)},
            {"Spaces", Parse.getSpaces(text)},
            {"Paragraphs", Parse.getParagraphs(text)},
            {"Pages", Parse.getPages(text)},
            {"Lines", 0},
            {"Sentences", Parse.getSentences(text)},
            {"Avg. Word (chars)", 0},
            {"Sentence Logest", 0},
            {"Sentence Shortest", 0},
