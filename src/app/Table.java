...
public class Table {
    ...
    private static DefaultTableModel getTable_DEF(String text) {
        ...
        Object[][] gridData = {
            {"Length", Parser.getSize(text)},
            {"Word Total", Parser.getWords(text)},
            {"Word Unique", Parser.getWordsUnique(text)},
            {"Characters", Parser.getCharacters(text)},
            {"Letters", Parser.getLetters(text)},
            {"UpperCase", Parser.getLettersUpper(text)},
            {"LowerCase", Parser.getLettersLower(text)},
            {"Numbers", Parser.getDigits(text)},
            {"Symbols", Parser.getSymbols(text)},
            {"Spaces", Parser.getSpaces(text)},
            {"Paragraphs", Parser.getParagraphs(text)},
            {"Pages", Parser.getPages(text)},
            {"Lines", 0},
